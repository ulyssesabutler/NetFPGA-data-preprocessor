...
for i in range(num_broadcast):
    ...
    sent_pkt = make_IP_pkt(src_MAC="aa:bb:cc:dd:ee:ff", dst_MAC=routerMAC[0],
                      EtherType=0x800, src_IP="192.168.0.1",
                      dst_IP="192.168.1.1", pkt_len=60)
    sent_pkt.time = ((i*(1e-8)) + (2e-6))
    sent_pkt_payload = bytearray(sent_pkt[Raw].load)

    # The expected packet, after being modified
    expected_pkt_payload = copy.deepcopy(sent_pkt_payload)
    for i in range(len(expected_pkt_payload)):
      expected_pkt_payload[i] = ~sent_pkt_payload[i] & 0xFF
    expected_pkt = copy.deepcopy(sent_pkt)
    expected_pkt[Raw].load = bytes(expected_pkt_payload)

    sent_pkts.append(sent_pkt)
    expected_pkts.append(expected_pkt)
    if isHW():
        nftest_expect_phy('nf1', expected_pkt)
        nftest_send_phy('nf0', sent_pkt)
    
if not isHW():
    nftest_send_phy('nf0', sent_pkts)
    nftest_expect_phy('nf0', expected_pkts)
